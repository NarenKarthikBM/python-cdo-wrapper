...
class TestCDOTextCommands:
    ...
    def test_text_commands_contains_common_operators(self):
        ...
        common_text_ops = [
            "sinfo", "info", "griddes", "showdate", "showname",
            "ntsteps", "nvars", "tinfo", "vlist",
        ]
    ...
    def test_text_commands_excludes_data_operators(self):
        ...
        data_ops = [
            "yearmean", "monmean", "selname", "remapbil",
            "fldmean", "add", "sub", "mul",
        ]
        for op in data_ops:
            assert op not in CDO_TEXT_COMMANDS, f"{op} should NOT be in CDO_TEXT_COMMANDS"
...
class TestCDOFunction:
    ...
    def test_cdo_data_command_returns_tuple(self, sample_nc_file: Path):
        ...
        with patch("python_cdo_wrapper.core.subprocess.run") as mock_run:
            ...
            with patch("python_cdo_wrapper.core.tempfile.NamedTemporaryFile") as mock_tmp:
    ...
    def test_cdo_return_xr_false(self):
        ...
        with patch("python_cdo_wrapper.core.subprocess.run") as mock_run:
            ...
            with patch("python_cdo_wrapper.core.tempfile.NamedTemporaryFile") as mock_tmp:
                ...
                with patch("python_cdo_wrapper.core.os.unlink"):
                    with patch("python_cdo_wrapper.core.os.path.exists", return_value=True):
